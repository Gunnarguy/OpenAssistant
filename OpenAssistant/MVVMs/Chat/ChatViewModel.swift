...
class ChatViewModel: BaseViewModel {
    ...
    private func loadMessagesForCurrentThread() {
        ...
        // Update the local messages array in natural order (oldest first, newest last)
    ...
    private func handleFetchMessagesResult(_ result: Result<[Message], OpenAIServiceError>) {
        ...
        case .success(let fetchedMessages):
            ...
            if !newLocalMessages.isEmpty {
                ...
                self.messages.append(contentsOf: sortedNewMessages)
    ...
    func sendMessage() {
        ...
        // Append user message to local array in natural order
    ...
    // Updated scrollToLastMessage to handle natural order and animation
    func scrollToLastMessage(animated: Bool = true) {
        guard let scrollViewProxy = scrollViewProxy, let lastMessage = messages.last else {
        ...
        // Scroll to the ID of the *last* message in the array (which is visually the last)
        DispatchQueue.main.async {  // Ensure UI updates on main thread
            withAnimation(animated ? .spring() : nil) {  // Use spring animation or none
                scrollViewProxy.scrollTo(lastMessage.id, anchor: .bottom)  // Anchor to bottom
            }
        }
    }
